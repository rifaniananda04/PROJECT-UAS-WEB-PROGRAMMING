...
$inf  = mysqli_query($koneksi,"SELECT * FROM tbl_informasi");
...
<head>
  ...
  <title>Sepeda Air Danau Kelapa Gading Admin-DIF_page.com</title>
...
<div class="container border border-2 rounded-3 p-2" style="margin-top: 120px;">
    <div class="row m-2">
      ...
      <h3 class="text-center">Data Tampilan Informasi</h3>
    </div>
        <div class="col-md-12">
    ...
    <tbody>
        ...
        <?php foreach ($inf as $data ) : ?> 
            <tr>
                ...
                <td><?= $data["email"]?></td>
                <td>
                    <button class="btn btn-sm btn-warning ms-2 me-2"><a href="ubh-inf.php?id=<?= $data["id"];?>" class="text-light">Ubah</a></button>
                    <!-- <button class="btn btn-sm btn-danger" ><a href="hps-inf.php?id=<?= $data["id"];?>" class="text-light" onclick="return confirm('Yakin ingin mengahpus informasi ini ?');">Hapus</a></button> -->
                </td>
            </tr>
        <?php $i++; ?>
        <?php endforeach; ?>
