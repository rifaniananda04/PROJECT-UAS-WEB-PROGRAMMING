...
INNER JOIN tb_lvl  WHERE users.id_akses = tb_lvl.id ");
...
<head>
  ...
  <title>Sepeda Air Danau Kelapa Gading Admin-DP_page.com</title>
    ...
    <tbody>
        ...
        <?php foreach ($p as $data ) : ?> 
            <tr>
                ...
                <td><?= $data["akses"]?></td>
                <td>
                    <button class="btn btn-sm btn-warning ms-2 me-2"><a href="ubh-pengguna.php?id=<?= $data["id"];?>" class="text-light">Ubah</a></button>
                    <button class="btn btn-sm btn-danger" ><a href="hps-pengguna.php?id=<?= $data["id"];?>" class="text-light" onclick="return confirm('Yakin ingin mengahpus user ini ?');">Hapus</a></button>
                </td>
            </tr>
        <?php $i++; ?>
        <?php endforeach; ?>
