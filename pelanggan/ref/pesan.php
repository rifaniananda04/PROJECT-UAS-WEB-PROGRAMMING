...
$psn  = mysqli_query($koneksi,"SELECT * FROM tbl_pesanan WHERE pemesan = '$id' ");
...
<head>
  ...
  <title>Sepeda Air Danau Kelapa Gading Costumer-DPSN_page.com</title>
...
<div class="container border border-2 rounded-3 p-2" style="margin-top: 120px;">
    <div class="row m-2">
      ...
      <h3 class="text-center">Data Seluruh Pesanan</h3>
    </div>
        <div class="col-md-12">
        ...
        <?php foreach ($psn as $data ) : ?> 
            ...
            <tr>
                ...
                <td class="text-center" >
                    <button class="btn btn-sm btn-warning "><a href="ubh-psn.php?id=<?= $data["id"];?>" class="text-light">Ubah</a></button>
                    <button class="btn btn-sm btn-danger" ><a href="hps-psn.php?id=<?= $data["id"];?>" class="text-light" onclick="return confirm('Yakin ingin mengahpus informasi ini ?');">Hapus</a></button>
                    <button class="btn btn-sm btn-success" ><a href="strk-psn.php?id=<?= $data["id"];?>" target="_blank" class="text-light">Struk</a></button>
                </td>
            </tr>
        <?php $i++; ?>
        <?php endforeach; ?>
